...
// const SATISFACTORY_DOCS_LOCATIONS = {
//   v600: './docs_v700.json',
//   v700: './docs_v700.json',
//   v800: './docs_v800.json',
//   v1000: './docs_v1000.json',
// };
...
export const getSatisfactoryData = (type, version) => {
  ...
  if (!satisfactory_data_new[version]) {
    ...
    if (localVersion === 'v1000') {
      ...
      data_v1000.items.forEach((item) => {
        const foundItem = keynames.find(
          (key) => satisfactory_data_new[localVersion].items[key]?.name === item.name
        );
  ...
  if (!type) {
    return satisfactory_data_new[localVersion] || satisfactory_data_new[SatisfactoryCurrentVersion];
  }
  return (
    satisfactory_data_new[localVersion][type] ||
    satisfactory_data_new[SatisfactoryCurrentVersion][type]
  );
};
...
export const getSatisfactoryEndProducts = (version) => {
  ...
  for (const [, recipeValue] of Object.entries(recipes)) {
...
export const getSatisfactoryAssetUrl = (type, className, version) => {
  const products = getSatisfactoryData(type, version);
  const productName = products[className];
  return `/assets/satisfactory/products/${productName}.png`;
};
