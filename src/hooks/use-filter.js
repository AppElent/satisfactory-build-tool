 ...
 */
function useFilter(initialData = [], options = {}) {
  const {
  ...
  const [data, setData] = useState(initialData); // Holds the current dataset
  const [page, setPage] = useState(initialPage); // Tracks the current page (0-based)
  const [rowsPerPage, setRowsPerPage] = useState(initialRowsPerPage); // Number of items to show per page
  const [sortField, setSortField] = useState(initialSortField); // Field used for sorting
  const [sortDirection, setSortDirection] = useState(initialSortDirection); // Sort direction ('asc' or 'desc')
